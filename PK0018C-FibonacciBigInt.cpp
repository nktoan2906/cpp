...
const long long base = 1000000000; const long long base_digits = 9;
struct bigint {
    vi a; long long sign;
    bigint() :
        sign(1) {
    ...
    static vll karatsubaMultiply(const vll &a, const vll &b) {
        ...
        if (n <= 32) {
            for (long long i = 0; i < n; i++){
                for (long long j = 0; j < n; j++) res[i + j] += a[i] * b[j];
			}
            return res;
        }
        long long k = n >> 1;
        ...
        for (long long i = 0; i < k; i++) a2[i] += a1[i];
        for (long long i = 0; i < k; i++) b2[i] += b1[i];
        vll r = karatsubaMultiply(a2, b2);
        for (long long i = 0; i < (long long) a1b1.size(); i++) r[i] -= a1b1[i];
        for (long long i = 0; i < (long long) a2b2.size(); i++) r[i] -= a2b2[i];
        for (long long i = 0; i < (long long) r.size(); i++) res[i + k] += r[i];
        for (long long i = 0; i < (long long) a1b1.size(); i++) res[i] += a1b1[i];
...
signed main() {
    FASTER;
    bigint f[1001] = {(bigint)1}, first = 1, second = 1, temp;
    f[1] = first;
    f[2] = second;
    long long i = 3;
    while(i <= 1000){
